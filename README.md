...
**Conjecture 1 — the full distributional law, now proved at K=1, 2, 3 (2026-08-24).** Beyond the mean `φ_∞(c)` above, the archive conjectures the *entire density* of the cyclic mass conditional on exactly `K` reroutes: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` closed in wave 14; `K=3` closed unexpectedly in wave 15, contradicting two prior fronts' diagnosis that the combinatorics of destination configurations would explode beyond `K=2` — instead, a structural fact ("off-cycle redirects contribute zero new cyclic mass") collapsed the raw configuration count down to a small, tractable number of cycle-shapes at `K=3` too. Each instance is independently hostile-refereed with zero mathematical errors found:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Plot of the three proved instances of Conjecture 1's density f_M_K(x) for K=1, 2, and 3" width="620"></p>

**The sharp-constant question — proved limit, open supremum.** `THEOREM.md` also proves `lim_{K→∞} M_K/√K = a* ≈ 0.367` exactly (Estágio 13) — the first rigorous confirmation that the numerically-conjectured sharp constant is genuinely the correct asymptotic value. Whether `sup_K M_K/√K` equals that same limit (i.e., whether `M_K/√K` never exceeds `a*` at any finite `K`) remains open — two routes have already failed, and a third is running now (wave 16, front b):

<p align="center"><img src="05_DISCOVERY_LAB/assets/sharp_constant_astar.svg" alt="Plot of M_K over square root of K approaching but never proved to be bounded by the sharp constant a-star" width="620"></p>
...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (adjudicated subset, `DISC-CORE-NUMERICS-001`) | **~97%** | ✅ Open Lemma and general-`K` rate conjecture proved **unconditionally for every `K≥0`** (2026-08-22); the exact, finite, all-orders, general-`K` closed form for the underlying recursion is now **proved** as well (2026-08-23), the convergence `φ(n,c)→φ_∞(c)` is proved **uniform in `c` on the entire range `[0,∞)`**, not just pointwise (2026-08-23), and an **explicit, unconditional convergence rate** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` is now proved as well (2026-08-23, non-sharp constant) — every earlier finite-`n` result on this line is a corollary of one formula | Remaining, none central: the M-CLUST(b) residual (a separate mechanism, `PARTIALLY CLOSED`); the full distributional law (Conjectures 1–2, now proved at `K=1,2,3` as of 2026-08-24 — the `K=3` closure was unexpected, contradicting a combinatorial-explosion diagnosis shared by two prior fronts; `K≥4` open, not attempted); a closed form for the sharp error constants (now proved for `p=1,...,10` at every `b≥0` as of 2026-08-24, with the underlying general-`k` machine proved correct for *every* `k` by induction — `p>10` open only because it wasn't executed, not from mathematical uncertainty); the **sharp** constant in the explicit rate above (`a*≈0.367` vs. the proved non-sharp `a≈2.253`, though the *limit* `lim_K M_K/√K=a*` is now proved exactly, 2026-08-23 — only the uniform-in-`K` supremum remains open) — see [dependency map](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
