...
**Conjectura 1 — a lei distribucional completa, agora provada em K=1, 2, 3 (2026-08-24).** Além da média `φ_∞(c)` acima, o arquivo conjectura a *densidade completa* da massa cíclica condicional a exatamente `K` reroteamentos: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` fechou na onda 14; `K=3` fechou inesperadamente na onda 15, contrariando o diagnóstico de duas frentes anteriores de que a combinatória de configurações de destino explodiria além de `K=2` — em vez disso, um fato estrutural ("redirecionamentos fora-do-ciclo contribuem massa cíclica nova exatamente zero") colapsou a contagem bruta de configurações a um número pequeno e tratável de formas de ciclo também em `K=3`. Cada instância foi verificada por um referee hostil independente, com zero erros matemáticos encontrados:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Gráfico das três instâncias provadas da densidade f_M_K(x) da Conjectura 1 para K=1, 2 e 3" width="620"></p>

**A questão da constante nítida — limite provado, supremo aberto.** `THEOREM.md` também prova `lim_{K→∞} M_K/√K = a* ≈ 0,367` exatamente (Estágio 13) — a primeira confirmação rigorosa de que a constante nítida numericamente conjecturada é genuinamente o valor assintótico correto. Se `sup_K M_K/√K` é igual a esse mesmo limite (isto é, se `M_K/√K` nunca ultrapassa `a*` em nenhum `K` finito) permanece aberto — duas rotas já falharam, e uma terceira está em andamento agora (onda 16, frente b):

<p align="center"><img src="05_DISCOVERY_LAB/assets/sharp_constant_astar.svg" alt="Gráfico de M_K sobre raiz quadrada de K se aproximando, mas sem prova de ser limitado pela constante nítida a-estrela" width="620"></p>
