...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (adjudicated subset, `DISC-CORE-NUMERICS-001`) | **~98%** | ✅ Open Lemma and general-`K` rate conjecture proved **unconditionally for every `K≥0`** (2026-08-22); the exact, finite, all-orders, general-`K` closed form for the underlying recursion is now **proved** as well (2026-08-23), the convergence `φ(n,c)→φ_∞(c)` is proved **uniform in `c` on the entire range `[0,∞)`**, not just pointwise (2026-08-23); the sharp explicit rate in `c` is fully **CLOSED**: `sup_K M_K/√K=a*≈0.367` proved (2026-08-25, third attempt), then assembled into `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n` for `n≥4`, `0≤c≤n`, strict, `κ_B` certified in pure rational arithmetic (2026-08-25); the `γ=c/n` scaling law is now **CLOSED** for every `γ∈(0,1]`, `φ(n,γn)/φ_∞(γn)→√(2/(2−γ))`, via a new exact finite-`n` double-sum formula, plus both bonus targets (uniformity on compacts, the `γ_n→0` limit) (2026-08-26); and the **full distributional law is now CLOSED for every `K≥1` at once** — Conjecture 1's density `f_{M_K}(x)=2Kx(1-x²)^{K-1}` and, as an indirect corollary via the Poisson-mixture algebra, Conjecture 2 itself (`M(c)=_d min(1,√(E/c))`) are both proved, closing `E[M(c)²]=(1−e^{−c})/c` and `E[M_K²]=1/(K+1)` unconditionally for every `K` (wave 17, 2026-08-26) — every earlier finite-`n` result on this line is a corollary of one formula | Remaining, none central: the M-CLUST(b) residual (a separate mechanism, `PARTIALLY CLOSED`, now with a genuinely-derived four-term asymptotic law for its plateau constant, `Π(c)=√(π/2c)−2/c+(7/2)√(π/2)·c^{−3/2}−(34/3)·c^{−2}+O(c^{−5/2})`, DERIVED heuristically and CONFIRMED numerically to ≥110 digits, not a full closed form — the first closed-form statement about this constant in the lineage, 2026-08-26); Conjecture 2's own *direct* route (not through Conjecture 1) remains honestly open — the joint two-point exploration machinery it was chartered to build has its first proved result (the Uniform Cyclic Restriction Theorem, 2026-08-26) but is still its own unresolved general sub-problem, even though the two numeric targets it originally targeted are now closed by the indirect route; wave 18 (2026-08-26) added a continuum theorem by transfer, `P(same final cycle | K marks)=1/(2(K+1))`, for `K=0,1` only (Estágio 28), and closed the `n→∞` distributional bridge `M_n(c)→_d M(c)` unconditionally for `K=0,1` with this line's first second-moment/fluctuation result (Estágio 27) — both leave **`K≥2` honestly open**; a closed form for the sharp error constants (proved and executed for `p=1,...,20` at every `b≥0`, `p>20` open only because it wasn't executed, not from mathematical uncertainty); a rigorous (non-heuristic) version of the `γ`-scaling law's second-order term `C(γ)` for **`γ∈(0,1)`** (wave 18 proved an exact equivalence, Lemma E, and the "deterministic half" closed form `D_0(γ)=(γ-1)/(2(2-γ))` unconditionally for every `γ∈(0,1]`, Estágio 26; the remaining "hard half" matches a heuristic derivation exactly and symbolically but is not proved), and the intermediate window between the fixed-`c` and `γ_n→0` regimes — see [dependency map](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
