...
|---:|---|---:|---|---|
| 🥇 | **D ― `U₁/₂`**(判定済みサブセット、`DISC-CORE-NUMERICS-001`) | **~97%** | ✅ 未解決補題(Open Lemma)と一般 `K` に対するレート予想は**すべての `K≥0` について無条件に**証明された(2026-08-22);根底にある再帰式について、一般 `K` に対する厳密かつ有限な全次数の閉形式も**証明された**(2026-08-23);収束 `φ(n,c)→φ_∞(c)` も、単なる各点収束ではなく**`c` について `[0,∞)` の全域で一様に**成り立つことが証明された(2026-08-23);さらに**明示的かつ無条件の収束レート** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` も証明された(2026-08-23、非最良定数)― 本ラインにおけるこれまでの有限 `n` の結果はすべて、この単一の式の系となる | 残されている非中心的な事項:M-CLUST(b)残差(別の機構、`PARTIALLY CLOSED`);完全な分布法則(予想1〜2、2026-08-24以降 `K=1,2,3` で証明済み ― `K=3` での成立は予想外であり、それ以前の2つの取り組みが共有していた組合せ爆発という診断に反する結果だった ― `K≥4` は未着手のまま未解決);厳密な誤差定数の閉形式(2026-08-24以降、`p=1,...,10`・任意の `b≥0` について証明済み、基盤となる一般 `k` の機構も帰納法によりすべての `k` で正しいと証明済み ― `p>10` は単に未実行なだけで未解決、数学的な不確実性によるものではない);上記の明示的レートにおける**最良(sharp)定数**(証明済みの非最良値 `a≈2.253` に対し `a*≈0.367`、ただし*極限* `lim_K M_K/√K=a*` 自体は2026-08-23に厳密に証明済み ― 残る未解決事項は `K` について一様な上限のみ) ― [依存関係マップ](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md)参照 |
