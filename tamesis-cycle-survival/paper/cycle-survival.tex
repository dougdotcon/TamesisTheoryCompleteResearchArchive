...
full for $K=0,1,2$ -- with exact rates $O(1/n^2)$ at $K=1$ and, more
surprisingly, only $\Theta(1/n)$ at $K=2$, not the naively extrapolated
$O(1/n^2)$; for $K\ge3$ it is reduced, via an unconditionally proved
mixing lemma, to a single precisely stated open convergence lemma, which
is not resolved here. Every claim below is labeled by its exact status;
...
closed form (Proposition~\ref{prop:k2}). For $K\ge3$ this case analysis
grows combinatorially: three or more reroutes can strike the same cycle
in any relative order, strike different cycles whose severed pieces
later re-link through a further reroute's target, or chase each other
through freshly explored territory in an order-dependent way that has no
$K=2$ analogue -- not resolved here.

\begin{openlemma}[Fixed-$K$ bridge, $K\ge3$]
\label{ol:k3}
$\displaystyle\lim_{n\to\infty}\varphi_n^{(K)} = \varphi_K$ for every
fixed integer $K\ge3$. \textbf{Neither proved nor disproved here.}
\end{openlemma}

\begin{proposition}[Conditional convergence]
\label{prop:conditional}
For every fixed $c\ge0$, $\varphi(n,c)\to\phiinf(c)$ as $n\to\infty$,
\textbf{conditional on} Open Lemma~\ref{ol:k3} holding for every
$K\ge3$.
\end{proposition}

This follows immediately from Proposition~\ref{prop:mixing} plus
Propositions~\ref{prop:k1} and \ref{prop:k2} and the trivial $K=0$ case,
given the open lemma. It is stated as a conditional proposition, not a
theorem, because the open lemma is exactly what is missing; the
empirical control that exists (\S\ref{sec:numerics}) is evidence for,
not a substitute for, resolving it. (Proposition~\ref{prop:mixing}'s
hypothesis -- fixed-$K$ convergence for \emph{every} $K\ge0$ -- is now
discharged unconditionally through $K=2$; the open lemma's scope has
strictly narrowed from $K\ge2$ to $K\ge3$.)
...
\begin{enumerate}
\item \textbf{Open Lemma~\ref{ol:k3}} itself: the fixed-$K$ finite-$n$
  bridge for $K\ge3$ (resolved unconditionally at $K=0,1,2$ by
  Propositions~\ref{prop:k1}--\ref{prop:k2}). This is the single
  hypothesis separating Proposition~\ref{prop:conditional} from a full
  theorem.
\item \textbf{The rate} of that bridge for $K\ge3$: now known exactly
  through $K=2$ -- $K=0$ is exact (rate $0$), $K=1$ is exact at rate
  $1/(3n^2)$, and $K=2$ is exact at rate $1/(30n)+7/(10n^2)+1/(5n^3)$,
  i.e.\ $\Theta(1/n)$, \emph{not} $\Theta(1/n^2)$
  (\S\ref{sec:numerics}). The jump from $\Theta(1/n^2)$ at $K=1$ to
  $\Theta(1/n)$ at $K=2$ is itself a caution against extrapolating any
  specific rate for $K\ge3$ without further work.
