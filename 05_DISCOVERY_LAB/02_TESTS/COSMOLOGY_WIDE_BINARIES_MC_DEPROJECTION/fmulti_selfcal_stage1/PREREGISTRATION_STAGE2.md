...
**Status:** LOCKED
**Data de criação do rascunho:** 2026-08-22 (`PREREGISTRATION_STAGE2_DRAFT.md`)
**Data de travamento:** 2026-08-22 (`DISC-DEC-029`)
**Autor (agente/sessão):** Tamesis Discovery Lab, sessão 2026-08-22 — rascunho
redigido por um front de agente (mandato `DISC-DEC-027`, item (a)); travado
pela sessão orquestradora após revisão explícita dos 4 itens assinalados na
Seção 12 — ver `DISC-DEC-029` em `00_GOVERNANCE/DECISION_LEDGER.yaml` para o
raciocínio de aprovação.
...
commit que registra `DISC-DEC-029` — ver `git log` para o hash exato.

> **Este documento é a versão TRAVADA**, promovida sem alteração de conteúdo
> matemático/metodológico de `PREREGISTRATION_STAGE2_DRAFT.md` (preservado
> integralmente nesta pasta como registro de auditoria) — apenas o cabeçalho
> acima e a Seção 12 (resoluções explícitas abaixo) foram atualizados.
> Nenhum dado real havia sido lido, aberto, importado ou computado antes
> deste lock. A partir deste lock, o cálculo real sobre a amostra de
> descoberta (30.203 sistemas, ainda NÃO o holdout selado) está autorizado,
> sujeito a toda disciplina desta especificação (Seções 6, 8, 9).
...
## 12. Judgment calls — RESOLVIDOS pela sessão orquestradora (`DISC-DEC-029`)

Os quatro itens abaixo foram revisados explicitamente antes deste lock, por
instrução de `DISC-DEC-027`. Texto original do rascunho preservado; cada
item recebe uma resolução datada.

> **[Resolução, 2026-08-22, `DISC-DEC-029`]** Todos os 4 itens abaixo
> aprovados como propostos pelo rascunho, com uma correção de código
> adicional (item 12.4): `calibrate_f_multi()` em `analysis/selfcal_pipeline.py`
> ganhou um parâmetro aditivo `return_raw: bool=False` (default inalterado)
> que propaga para sua chamada interna final e inclui o dict `_raw`
> resultante na chave `"final_raw"` do retorno — elimina inteiramente o
> risco de descasamento de semente identificado no item 12.4/Seção 4.5, ao
> invés de apenas documentá-lo como uma disciplina manual a seguir. A
> revalidação obrigatória da Seção 9.4 (cenário `f_multi_true=0,35` da
> Validação B sob `N_bootstrap=2000`) foi executada nesta revisão: o IC95%
> mais preciso é `[1,4727×10⁻¹⁰; 2,6108×10⁻¹⁰]` — **ainda NÃO contém**
> `a0_true=1,20×10⁻¹⁰` (era `[1,4722×10⁻¹⁰; 2,6403×10⁻¹⁰]` sob
> `N_bootstrap=300`) — a margem "por pouco" é estável sob 5× mais réplicas
> de bootstrap, não um artefato de baixa precisão de percentil; fortalece a
> leitura de que o resíduo é um viés genuíno pequeno, não ruído de
> estimação de IC. **Checklist completo da Seção 9 executado nesta revisão
> (não só o item 9.4):** 9.1 (guarda `fit_a0>0` presente — confirmada, é a
> mesma adicionada nesta revisão); 9.2 (`validate_a_recover_f_multi.py`, os
> 3 cenários, e `validate_b_recover_a0_with_contamination.py`, os 2
> cenários, reexecutados sob `N_bootstrap=2000` — os 7 critérios A1-A4/B1-B3
> passam nos 5 cenários; cenário B `f_multi_true=0,47` a `N_bootstrap=2000`:
> `a0_fit_corr=1,7238×10⁻¹⁰`, IC95% `[1,1061×10⁻¹⁰;2,4861×10⁻¹⁰]`, contém
> `a0_true=1,2×10⁻¹⁰` com folga maior que a `N_bootstrap=300` original); 9.3
> (grep de todo `.py` desta pasta pelos 4 nomes de arquivo de dado real
> travados — zero ocorrências fora de docstring); 9.5 (checagem a priori de
> insensibilidade do bin-âncora — `negligible: true` confirmado em todos os
> cenários rodados). **Critério de bloqueio da Seção 9 não acionado.**
