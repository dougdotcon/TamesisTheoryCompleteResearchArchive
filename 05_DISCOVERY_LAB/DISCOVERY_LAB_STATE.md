...
**Última atualização:** 2026-08-26 (`DISC-DEC-074`: integração da
onda 17 frente (e) — `GAMMA-SCALING-LAW-ATTEMPT` CONCLUÍDA e
INTEGRADA, após verificação adversarial hostil dedicada (veredito
**SOUND**, "ACCEPT for catalogue"). **Primeiro ataque dedicado ao
item aberto desde os Estágios 10–13 fecha o MANDATO COMPLETO**:
Teorema 2 — `φ(n,γn)/φ_∞(γn) → √(2/(2−γ))` — provado para **todo**
`γ∈(0,1]` fixo, taxa `O_γ(n^{−1/4})` via um sanduíche explícito em
`n` finito; ambos os alvos-bônus também provados — uniformidade em
compactos `[γ₀,1]` (Corolário 1) e o limite `γ_n→0` com
`γ_n n^{1/3}/\ln n→∞` (Corolário 2). O motor é uma **nova fórmula
soma-dupla exata em `n` finito** (Lema 1), derivada do zero da
Definição 1 — deliberadamente NÃO usa a maquinaria dos Estágios
9/12/22, diagnosticada e confirmada estruturalmente fraca demais para
a razão relativa neste regime (`O(1)` relativo vs. `φ_∞(γn) =
Θ(n^{−1/2})`). Bônus honesto: o termo de segunda ordem `C(γ)` provado
em `γ=1` (reduz a `−2/(3√π)`), **conjecturado** — não provado — em
`γ∈(0,1)`. O referee re-derivou o Lema 1 à mão antes de escrever
qualquer código (confirmação genuína, não leitura), reconstruiu todo
o motor numérico do zero, replicou a força bruta exata em `n=3,4,5`
e ~154.000 checagens pontuais (0 violações após corrigir um bug de
underflow no próprio script). A sessão fez seu spot-check antes de
catalogar e — na tentativa — encontrou e corrigiu um bug de
underflow catastrófico no seu **próprio** verificador, da mesma
classe do bug que o referee já havia documentado; após a correção,
todos os 6 valores de `γ` da tabela bateram a ~10⁻¹¹. Integrado como
"Estágio 23" em `THEOREM.md`, pointer datado na seção original do
Estágio 10. Onda 17: 2/5 integradas; frentes (a) e (c) com
resultados pendentes de integração, (d) em pesquisa. Anterior:
`DISC-DEC-073`: integração da
