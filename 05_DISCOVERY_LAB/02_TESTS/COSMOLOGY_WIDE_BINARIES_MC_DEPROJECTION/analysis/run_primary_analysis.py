"""
DISC-COSMOLOGY-MOND-SPARC-004 -- analise primaria pre-registrada, PRIMEIRA
vez que dado real de velocidade observada (v_p) e' tocado neste teste.

Executa EXATAMENTE a estatistica de teste descrita em PREREGISTRATION.md
(Secoes 4/4c, LOCKED em 2026-08-15) sobre a amostra de DESCOBERTA real
(30.203 sistemas binarios largos do Gaia EDR3, El-Badry, Rix & Heintz 2021,
apos cortes de qualidade + cruzamento com excentricidades de Hwang, Ting &
Zakamska 2022). O holdout (12.944 sistemas) NUNCA e' lido -- so'
discovery_pair_ids e' usado, e a disjuncao com holdout_pair_ids e'
verificada programaticamente abaixo.

NAO edita deprojection_common.py nem delta_obs_newt.py (PROIBIDO) -- so'
CHAMA as funcoes publicas ja travadas la (dc.run_mc_deprojection,
don.run_delta_obs_newt, don.generate_synthetic_vp_newtonian,
don.projected_log_gN, don.assign_bins_by_projected_gN, don.delta_aqual).
...
OUT_JSON = THIS_DIR / "result_primary.json"
...
def main():
    ...
    print(f"\n[3] Rodando don.run_delta_obs_newt (n_mc={N_MC}, "
          ...
          f"pipeline travada, sem modificacao...")
    ...
    result = {
        ...
        "analysis_script": (
            "05_DISCOVERY_LAB/02_TESTS/COSMOLOGY_WIDE_BINARIES_MC_DEPROJECTION/analysis/run_primary_analysis.py"
        ),
        "locked_pipeline_files_used_not_edited": [
        ...
        "note_first_time_real_vp_used": (
            "Esta e' a PRIMEIRA execucao deste teste sobre v_p observado real "
            "(velocidade projetada real, calculada de pmRA/pmDE/distancia "
            "reais do catalogo El-Badry+2021) -- toda validacao/revalidacao "
            "anterior (validation_synthetic_newtonian.json, "
            "revalidation_delta_obs_newt.json) usou somente v_p sintetico "
            "(Newtoniano puro, com ou sem boost MOND injetado) em AMBOS os "
            "ramos (real e mock)."
        ),
