...
**予想1 ― 完全な分布法則、K=1, 2, 3, 4で証明済み(2026-08-25)。** 上記の平均 `φ_∞(c)` に加えて、本アーカイブはちょうど `K` 回の迂回を条件とする巡回質量の*完全な密度*を予想している:`f_{M_K}(x) = 2Kx(1-x²)^(K-1)`。`K=2` はウェーブ14で決着し、`K=3` はウェーブ15で予想外に決着した ― これは、目的地の組み合わせが `K=2` を超えると爆発的に増加するという、それ以前の2つの研究戦線による診断に反する結果である。代わりに、「サイクル外のリダイレクトは新たな巡回質量への寄与が正確にゼロである」という構造的事実により、生の配置数が `K=3` においても小規模で扱いやすいサイクル形状の数へと収束した。`K=4` はウェーブ16で決着した ― 2度連続の予想外の結果である:Bell(4)=15 個の一致パターンは、分割↔置換の全単射を通じて、重み `∏(b_j−1)!` の総和がちょうど `K!=24` となるわずか5つの形状へと収束し、サイクル外の相殺の背後にある重み付き森の恒等式 `E(E+Q)^{n−1}=E` は、今や一般の `K≥5` に対する名指しされた候補ルートとなっている(未解決、未着手)。各インスタンスは独立した敵対的査読者によって検証され、数学的な誤りは一切見つかっていない:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="K=1, 2, 3, 4について証明された予想1の密度f_M_K(x)の4つのインスタンスのグラフ" width="620"></p>

**最良定数 ― 上限は今や証明済み:`sup_K M_K/√K = a*`(2026-08-25)。** `THEOREM.md` は `lim_{K→∞} M_K/√K = a* ≈ 0.367` を厳密に証明しており(Estágio 13)、2つのルートの失敗を経た3回目の試みで、ウェーブ16のフロント(b)が上限も決着させた:すべての `K≥1` について `M_K < a*√K` が*厳密に*成り立ち、したがって上限は極限に等しい(Estágio 19 ― Robbins 1955、Ramanujan `Q` 関数に対するFlajolet–Grabner–Kirschenhofer–Prodingerの両側評価、および古典的な厳密恒等式による;境界ケースは敵対的査読者自身によって決着され、独立に再検証された)。仮説(U')は今や、非最良の `a=1+√(π/2)≈2.253` に代わって最良定数 `a*` を保持している。このサブラインに残された唯一の課題は、今や完全に**CLOSED**である:Estágio 12の組み立ては同日(ウェーブ17)のうちに `a*` を用いて再実行され、`n≥4`、`0≤c≤n` において `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n` が得られ、`(0,n]` 上では厳密に成り立つ。加法定数は((U')からのポアソン混合半分の構造的独立性により)不変のままであり、今や純粋な有理数演算によって証明されている ― さらにこの評価は `c=n` に沿って漸近的にタイトであり、したがって `a*` はこの形の評価に対して可能な最良の乗法定数である:
...
|---:|---|---:|---|---|
| 🥇 | **D ― `U₁/₂`**(判定済みサブセット、`DISC-CORE-NUMERICS-001`) | **~97%** | ✅ 未解決補題(Open Lemma)と一般 `K` に対するレート予想は**すべての `K≥0` について無条件に**証明された(2026-08-22);根底にある再帰式について、一般 `K` に対する厳密かつ有限な全次数の閉形式も**証明された**(2026-08-23);収束 `φ(n,c)→φ_∞(c)` も、単なる各点収束ではなく**`c` について `[0,∞)` の全域で一様に**成り立つことが証明された(2026-08-23);さらに**明示的かつ無条件の収束レート** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` も証明された(2026-08-23、非最良定数)― 本ラインにおけるこれまでの有限 `n` の結果はすべて、この単一の式の系となる | 残されている非中心的な事項:M-CLUST(b)残差(別の機構、`PARTIALLY CLOSED`);完全な分布法則(予想1は2026-08-25以降 `K=1,2,3,4` で証明済み ― `K=3` と `K=4` の決着は、組合せ爆発という診断に反する連続した予想外の結果だった;`K≥5` は名指しされた候補ルート、すなわちEstágio 20の重み付き森の恒等式を伴って未解決;予想2の直接ルートはウェーブ16でも正直にクローズされて**いない** ― モーメント法のアーキテクチャは構築され、`c` に関するポアソン化のショートカットは厳密な反例によって*反証*され、真の障害は2点同時探索にあると特定された);厳密な誤差定数の閉形式(2026-08-25以降、`p=1,...,20`・任意の `b≥0` について証明・実行済み、75,899件の照合による一様検証を伴い、査読者によるおまけとして次数評価 `deg_r H_{2k−1}=k−1` も証明済み ― `p>20` は単に未実行なだけで未解決、数学的な不確実性によるものではない);`c` に関する最良の明示的レート ― 今や完全に**CLOSED**:上限 `sup_K M_K/√K=a*≈0.367` は2026-08-25に(3回目の試みで)**PROVED**され、続いてEstágio 12の `a*` による再組み立てが同日(ウェーブ17)に実行され、敵対的に検証された:`n≥4`、`0≤c≤n` について `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n` が厳密に成り立ち、加法定数は不変で純粋な有理数演算によって証明されており、この評価は `c=n` に沿って漸近的にタイトである ― [依存関係マップ](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md)参照 |
