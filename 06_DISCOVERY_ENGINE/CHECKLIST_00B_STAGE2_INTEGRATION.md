...
- [ ] `tests/test_stage2_integration.py`: a single scenario exercising at
...
- [ ] `pytest 06_DISCOVERY_ENGINE/tests/ -v` — the FULL suite (Stage 1 +
...
- [ ] A short adversarial review pass, by an agent that did not write
      ...
      `04_FORMAL_RESEARCH_LAB/` from `lean_bridge.py`.
- [ ] `GOAL.md`'s Stage 2 row is not marked done until every box above
      (and every Module 6–10 checklist) is genuinely checked, mirroring
      the same discipline already applied to Stage 1.
