...
| Adjudicación numérica del núcleo | **Consolidación matemática completa — el Lema Abierto ahora está probado incondicionalmente para todo `K` (2026-08-22)** | Ver más abajo — 3 afirmaciones cerradas en negativo con veredicto final; la 4ª (`U₁/₂`) tiene un núcleo probado, arbitrado adversarialmente tres veces de forma independiente, con el Lema Abierto y la conjetura de tasa ahora probados incondicionalmente para **todo** `K≥0` (no solo `K=0,…,10`) — la última salvedad de regularidad nombrada se cerró mediante una demostración de existencia por Gronwall discreto, y la proposición condicional del archivo fue promovida a un teorema incondicional (**Teorema 3**). |
...
| **Ley límite `U₁/₂` (oleadas 2–8, consolidada)** | 1 teorema + 1 generalización + Lema Abierto, todo `K≥0` + conjetura de tasa para `K` general, ambos incondicionales | **Probado, arbitrado adversarialmente (4 rondas independientes, técnicas distintas), publicado como artículo + paquete reproducible; `K=2` probado en la oleada 5, `K=3,4,5` en la oleada 6, `K=6,…,10` en la oleada 7 mediante el método de matriz de transferencia; la tasa para `K` general fue probada primero condicionalmente en la oleada 7, y luego la propia salvedad de regularidad se cerró en la oleada 8 mediante una demostración de existencia por Gronwall discreto — el Lema Abierto y la conjetura de tasa ahora están PROBADOS incondicionalmente para todo `K`, promoviendo la proposición condicional del archivo a un teorema incondicional** (ver más abajo) |
...
**Conjetura 1 — la ley distribucional completa, ahora probada en K=1, 2, 3, 4 (2026-08-25).** Además de la media `φ_∞(c)` anterior, el archivo conjetura la *densidad completa* de la masa cíclica condicional a exactamente `K` reencaminamientos: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` se cerró en la onda 14; `K=3` se cerró inesperadamente en la onda 15, contradiciendo el diagnóstico de dos frentes anteriores de que la combinatoria de configuraciones de destino explotaría más allá de `K=2` — en cambio, un hecho estructural ("los redireccionamientos fuera del ciclo contribuyen exactamente cero masa cíclica nueva") colapsó el recuento bruto de configuraciones a un número pequeño y manejable de formas de ciclo también en `K=3`. `K=4` se cerró en la onda 16 — una segunda sorpresa consecutiva: los Bell(4)=15 patrones de coincidencia colapsan a solo 5 formas cuyos pesos `∏(b_j−1)!` suman exactamente `K!=24` mediante una biyección partición↔permutación, y la identidad de bosques ponderados `E(E+Q)^{n−1}=E` detrás de la cancelación fuera del ciclo es ahora la ruta candidata nombrada para `K≥5` general (abierta, no intentada). Cada instancia fue verificada por un árbitro hostil independiente, sin ningún error matemático encontrado:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Gráfico de las cuatro instancias probadas de la densidad f_M_K(x) de la Conjetura 1 para K=1, 2, 3 y 4" width="620"></p>

**La constante nítida — supremo ahora PROBADO: `sup_K M_K/√K = a*` (2026-08-25).** `THEOREM.md` prueba `lim_{K→∞} M_K/√K = a* ≈ 0,367` exactamente (Estágio 13) y, en el tercer intento tras dos rutas fallidas, el frente (b) de la onda 16 cerró también el supremo: `M_K < a*√K` *estrictamente* para todo `K≥1`, de modo que el supremo es igual al límite (Estágio 19 — vía Robbins 1955, la cota bilateral de Flajolet–Grabner–Kirschenhofer–Prodinger sobre la función `Q` de Ramanujan y una identidad exacta clásica; el caso frontera fue cerrado por el propio árbitro hostil y re-verificado de forma independiente). La Hipótesis (U') lleva ahora la constante nítida `a*` en lugar de la no nítida `a=1+√(π/2)≈2,253`. La única pieza que quedaba en esta sublínea está ahora completamente CERRADA: el ensamblaje del Estágio 12 se re-ejecutó con `a*` ese mismo día (onda 17), dando `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0,2805]/n` para `n≥4`, `0≤c≤n`, estricta en `(0,n]`, con la constante aditiva sin cambios (independencia estructural de la mitad de la mezcla de Poisson respecto de (U')) y ahora certificada en aritmética racional pura — y la cota es asintóticamente ajustada a lo largo de `c=n`, de modo que `a*` es la mejor constante multiplicativa posible para esta forma de cota:
...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (subconjunto adjudicado, `DISC-CORE-NUMERICS-001`) | **~97%** | ✅ Lema Abierto y conjetura de tasa para `K` general probados **incondicionalmente para todo `K≥0`** (2026-08-22); la forma cerrada exacta, finita, a todos los órdenes, para `K` general, de la recursión subyacente también está **probada** (2026-08-23), la convergencia `φ(n,c)→φ_∞(c)` está probada **uniforme en `c` en todo el rango `[0,∞)`**, no solo puntual (2026-08-23), y una **tasa de convergencia explícita e incondicional** `|Δ_n(c)|≤[(1+√(π/2))√c+0,2805]/n` también está ahora probada (2026-08-23, constante no nítida) — todo resultado anterior en `n` finito de esta línea es ahora un corolario de una única fórmula | Restante, ninguno central: el residuo M-CLUST(b) (un mecanismo separado, `PARTIALLY CLOSED`); la ley distribucional completa (Conjetura 1 ahora probada en `K=1,2,3,4` desde 2026-08-25 — los cierres en `K=3` y `K=4` fueron sorpresas consecutivas frente a un diagnóstico de explosión combinatoria; `K≥5` abierto con una ruta candidata nombrada, la identidad de bosques ponderados del Estágio 20; la ruta directa de la Conjetura 2 honestamente NO cerrada en la onda 16 — arquitectura del método de momentos construida, el atajo de Poissonización en `c` *refutado* por un contraejemplo exacto, y la obstrucción genuina localizada en la exploración conjunta de dos puntos); una forma cerrada para las constantes de error nítidas (ahora probada y ejecutada para `p=1,...,20` en todo `b≥0` desde 2026-08-25, verificación uniforme de 75,899 comprobaciones, con la cota de grado `deg_r H_{2k−1}=k−1` probada como bonus del árbitro — `p>20` abierto solo por no haberse ejecutado, no por incertidumbre matemática); la tasa explícita nítida en `c` — ahora completamente **CERRADA**: el supremo `sup_K M_K/√K=a*≈0,367` fue PROBADO el 2026-08-25 (tercer intento), y el re-ensamblaje del Estágio 12 con `a*` se ejecutó luego y se verificó adversarialmente ese mismo día (onda 17): `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0,2805]/n` para `n≥4`, `0≤c≤n`, estricta, con la constante aditiva sin cambios y certificada en aritmética racional pura, y la cota asintóticamente ajustada a lo largo de `c=n` — ver [mapa de dependencias](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
