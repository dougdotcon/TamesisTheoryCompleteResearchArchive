...
**Conjecture 1 — the full distributional law, now proved at K=1, 2, 3, 4 (2026-08-25).** Beyond the mean `φ_∞(c)` above, the archive conjectures the *entire density* of the cyclic mass conditional on exactly `K` reroutes: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` closed in wave 14; `K=3` closed unexpectedly in wave 15, contradicting two prior fronts' diagnosis that the combinatorics of destination configurations would explode beyond `K=2` — instead, a structural fact ("off-cycle redirects contribute zero new cyclic mass") collapsed the raw configuration count down to a small, tractable number of cycle-shapes at `K=3` too. `K=4` closed in wave 16 — a second consecutive surprise: the Bell(4)=15 coincidence patterns collapse to just 5 shapes whose weights `∏(b_j−1)!` sum to exactly `K!=24` via a partition↔permutation bijection, and the weighted-forest identity `E(E+Q)^{n−1}=E` behind the off-cycle cancellation is now the named candidate route for general `K≥5` (open, not attempted). Each instance is independently hostile-refereed with zero mathematical errors found:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Plot of the four proved instances of Conjecture 1's density f_M_K(x) for K=1, 2, 3, and 4" width="620"></p>

**The sharp constant — supremum now PROVED: `sup_K M_K/√K = a*` (2026-08-25).** `THEOREM.md` proves `lim_{K→∞} M_K/√K = a* ≈ 0.367` exactly (Estágio 13), and, on the third attempt after two failed routes, wave 16 front (b) closed the supremum as well: `M_K < a*√K` *strictly* for every `K≥1`, so the supremum equals the limit (Estágio 19 — via Robbins 1955, the Flajolet–Grabner–Kirschenhofer–Prodinger two-sided bound on the Ramanujan `Q`-function, and a classical exact identity; the boundary case was closed by the hostile referee itself and independently re-verified). Hypothesis (U') now carries the sharp constant `a*` in place of the non-sharp `a=1+√(π/2)≈2.253`. The one piece left on this sub-line is mechanical and honestly catalogued as not yet executed: re-running the Estágio 12 assembly with `a*` to state the sharp explicit rate in `c`:
...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (adjudicated subset, `DISC-CORE-NUMERICS-001`) | **~97%** | ✅ Open Lemma and general-`K` rate conjecture proved **unconditionally for every `K≥0`** (2026-08-22); the exact, finite, all-orders, general-`K` closed form for the underlying recursion is now **proved** as well (2026-08-23), the convergence `φ(n,c)→φ_∞(c)` is proved **uniform in `c` on the entire range `[0,∞)`**, not just pointwise (2026-08-23), and an **explicit, unconditional convergence rate** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` is now proved as well (2026-08-23, non-sharp constant) — every earlier finite-`n` result on this line is a corollary of one formula | Remaining, none central: the M-CLUST(b) residual (a separate mechanism, `PARTIALLY CLOSED`); the full distributional law (Conjecture 1 now proved at `K=1,2,3,4` as of 2026-08-25 — the `K=3` and `K=4` closures were consecutive surprises against a combinatorial-explosion diagnosis; `K≥5` open with a named candidate route, the weighted-forest identity of Estágio 20; Conjecture 2's direct route honestly NOT closed in wave 16 — moment-method architecture built, the Poissonization-in-`c` shortcut *refuted* by exact counterexample, and the genuine obstruction located at joint two-point exploration); a closed form for the sharp error constants (now proved and executed for `p=1,...,20` at every `b≥0` as of 2026-08-25, 75,899-check uniform verification, with the degree bound `deg_r H_{2k−1}=k−1` proved as a referee bonus — `p>20` open only because it wasn't executed, not from mathematical uncertainty); the sharp explicit rate in `c` — now fully **CLOSED**: the supremum `sup_K M_K/√K=a*≈0.367` was PROVED on 2026-08-25 (third attempt), and the Estágio 12 re-assembly with `a*` was then executed and adversarially verified the same day (wave 17): `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n` for `n≥4`, `0≤c≤n`, strict, with the additive constant unchanged and certified in pure rational arithmetic, and the bound asymptotically tight along `c=n` — see [dependency map](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
