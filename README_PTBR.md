...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (subconjunto adjudicado, `DISC-CORE-NUMERICS-001`) | **~94%** | ✅ Lema Aberto e conjectura de taxa para `K` geral provados **incondicionalmente para todo `K≥0`** (2026-08-22); a forma fechada exata, finita, em todas as ordens, para `K` geral, da recursão subjacente também está **provada** (2026-08-23), e a convergência `φ(n,c)→φ_∞(c)` é provada **uniforme em `c` em toda a faixa `[0,∞)`**, não apenas pontual (2026-08-23) — todo resultado anterior em `n` finito nesta linha agora é um corolário de uma única fórmula | Restante, nenhum central: o resíduo M-CLUST(b) (um mecanismo separado, `PARTIALLY CLOSED`); a lei distribucional completa (Conjecturas 1–2); uma forma fechada para as constantes de erro precisas em `b≥2`; uma taxa de convergência explícita (condicional a uma hipótese de uniformidade nomeada e não provada) — ver [mapa de dependência](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
