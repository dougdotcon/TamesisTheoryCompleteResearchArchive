...
L = sp.symbols('L0 L1 L2 L3 L4 L5', positive=True)
...
def P_pair_sym(s, sp_, x):
    ...
    for mask in range(3 ** m):
        ...
        for j in range(m):
            d = code % 3
            code //= 3
            if d == 0: S1.append(M[j])
            elif d == 1: S2.append(M[j])
        p1 = sp.Integer(1)
        for u in S1: p1 *= x[u]
        p2 = sp.Integer(1)
        for u in S2: p2 *= x[u]
...
if __name__ == '__main__':
    t0 = time.time()
    print("Building T(L0..L5) symbolically...", flush=True)
    T = build_T()
    print(f"  done in {time.time()-t0:.1f}s, ops={sp.count_ops(T)}", flush=True)
    expr = T
    bounds = [
        (L[5], 1, n - L[0]-L[1]-L[2]-L[3]-L[4]),
        (L[4], 1, n - L[0]-L[1]-L[2]-L[3]-1),
        (L[3], 1, n - L[0]-L[1]-L[2]-2),
        (L[2], 1, n - L[0]-L[1]-3),
        (L[1], 1, n - L[0]-4),
        (L[0], 1, n - 5),
    ]
    ...
    binom_n6 = sp.factorial(n) / (sp.factorial(6) * sp.factorial(n - 6))
    denom = binom_n6 * (n - 6) * (n - 7)
    P_nn_6 = sp.simplify(expr / denom)
    P_nn_6 = sp.factor(sp.together(P_nn_6))
    print("FINAL Proposition NN6 (symbolic):", flush=True)
    print(P_nn_6, flush=True)
