...
**猜想1——完整的分布律,现已在 K=1、2、3、4 得证(2026-08-25)。** 除了上文的均值 `φ_∞(c)` 之外,档案库还猜想了在恰好经历 `K` 次改道的条件下,循环质量的*完整密度*:`f_{M_K}(x) = 2Kx(1-x²)^(K-1)`。`K=2` 已在第14轮研究中闭合;`K=3` 在第15轮研究中出人意料地闭合——这与此前两条研究战线关于目标配置组合会在 `K=2` 之后发生爆炸性增长的诊断相悖。相反,一个结构性事实("离环重定向对新增循环质量的贡献恰好为零")使得原始配置数在 `K=3` 时同样坍缩为数量少且可处理的循环形状。`K=4` 已在第16轮研究中闭合——这是连续第二次出人意料的结果:Bell(4)=15 种重合模式(coincidence patterns)通过一个分拆↔置换双射(partition↔permutation bijection)坍缩为仅仅 5 种形状,其权重 `∏(b_j−1)!` 之和恰好等于 `K!=24`;而支撑离环相消的加权森林恒等式(weighted-forest identity)`E(E+Q)^{n−1}=E`,现已成为一般 `K≥5` 情形的具名候选路径(仍属未解决,尚未尝试)。每一个实例都经过独立的敌对评审验证,未发现任何数学错误:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="猜想1的密度f_M_K(x)在K=1、2、3、4处四个已证明实例的图像" width="620"></p>

**最优常数——上确界现已证明:`sup_K M_K/√K = a*`(2026-08-25)。** `THEOREM.md` 精确证明了 `lim_{K→∞} M_K/√K = a* ≈ 0.367`(Estágio 13);而在两条路径失败后的第三次尝试中,第16轮研究的战线(b)也闭合了上确界:对每一个 `K≥1`,`M_K < a*√K` *严格*成立,因此上确界等于该极限(Estágio 19——借助 Robbins 1955、Flajolet–Grabner–Kirschenhofer–Prodinger 关于 Ramanujan `Q` 函数的双侧界,以及一个经典的精确恒等式;边界情形由敌对评审员亲自闭合,并经独立重新验证)。假设 (U') 现在携带的是最优常数 `a*`,取代了非最优的 `a=1+√(π/2)≈2.253`。这条子研究线上唯一剩下的部分现已**完全闭合**:同一天(第17轮研究),Estágio 12 的组装已用 `a*` 重新执行,给出 `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n`,对 `n≥4`、`0≤c≤n` 成立,且在 `(0,n]` 上*严格*成立;加性常数保持不变(泊松混合部分相对于 (U') 的结构性独立性),现已通过纯有理数运算认证——且该界沿 `c=n` 渐近紧,因此 `a*` 是这一形式的界所能达到的最佳乘法常数:
...
|---:|---|---:|---|---|
| 🥇 | **D —— `U₁/₂`**(已裁定子集,`DISC-CORE-NUMERICS-001`) | **约 97%** | ✅ 开放引理与一般 `K` 的速率猜想已**对每一个 `K≥0` 无条件证明**(2026-08-22);关于底层递归式、适用于一般 `K` 的精确、有限的全阶闭式表达式现已同样得到**证明**(2026-08-23),收敛 `φ(n,c)→φ_∞(c)` 已被证明在 **`c` 取遍整个范围 `[0,∞)` 时一致成立**,而不仅仅是逐点成立(2026-08-23),且一个**显式、无条件的收敛速率** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` 现已同样得到证明(2026-08-23,非最优常数)——本研究线此前所有有限 `n` 的结果现均成为这一单一公式的推论 | 剩余事项,均非核心:M-CLUST(b) 残差(一个独立的机制,`PARTIALLY CLOSED`);完整的分布律(猜想 1 自 2026-08-25 起已在 `K=1,2,3,4` 得证——`K=3` 与 `K=4` 的闭合是与组合爆炸诊断相悖的连续两次意外;`K≥5` 仍属未解决,但已有具名候选路径,即 Estágio 20 的加权森林恒等式;猜想 2 的直接路径在第16轮研究中被如实报告为未闭合——矩方法(moment-method)框架已搭建,`c` 上的泊松化(Poissonization)捷径被精确反例*驳倒*,真正的障碍则被定位在两点联合探索处);精确误差常数的闭式表达式(自 2026-08-25 起已对 `p=1,...,20`、任意 `b≥0` 证明并执行,完成了 75,899 次核对的一致验证,且次数界 `deg_r H_{2k−1}=k−1` 作为评审员的额外收获一并得证——`p>20` 仅因尚未执行而未解决,并非数学上存在不确定性);以 `c` 表示的最优显式速率——现已**完全闭合**:上确界 `sup_K M_K/√K=a*≈0.367` 已于 2026-08-25(第三次尝试)**得到证明**,同一天(第17轮研究)又完成了用 `a*` 进行的 Estágio 12 重新组装,并经敌对评审验证:`|φ(n,c)−φ_∞(c)| ≤ [a*√c+0.2805]/n` 对 `n≥4`、`0≤c≤n` 成立,*严格*不等式,加性常数保持不变,并已通过纯有理数运算认证,且该界沿 `c=n` 渐近紧——见[依赖关系图](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
