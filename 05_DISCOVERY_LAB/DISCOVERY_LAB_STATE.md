...
**Última atualização:** 2026-08-20 (Fase 0.7 de `DISC-TRI-RG-001`: nova rodada de busca a pedido do usuário, reabrindo a linha logo após `DISC-DEC-008`; 3 novos candidatos `viable=true` — Lempel-Ziv, Lyapunov, DMD/Koopman — e 2 `viable=false` por identificabilidade rigorosa — MF-DFA, EMD/HHT)
...
| Próxima ação obrigatória | Nenhuma obrigatória. `DISC-TRI-RG-001` reaberta na prática (Fase 0.7, 2026-08-20) — 3 candidatos novos `viable=true` (complexidade de Lempel-Ziv [rank 1], maior expoente de Lyapunov [rank 2], DMD/espectro de Koopman [rank 3]) aguardando escolha do usuário sobre qual fechar primeiro; nenhum ainda travado em `METHODOLOGY_NOTE.md` nem testado contra dado real; status permanece `CANDIDATE_FORMULATING`. Ver `02_TESTS/TRI_RG/phase0/PHASE0_7_SURVEY_NEW_CANDIDATES.md` |
