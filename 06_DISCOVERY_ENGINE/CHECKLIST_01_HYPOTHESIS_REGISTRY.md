...
- [x] `Claim` object with fields: `id` (`DISC-YYYY-NNNNN`), `title`,
...
- [x] State machine enum/constants matching `ROADMAP.md` exactly:
      `DRAFT → PRE_REGISTERED → LOCKED → RUNNING → RESULT →
      ADVERSARIAL_REVIEW → {CONFIRMED | REFUTED | INCONCLUSIVE | NULL}`.
- [x] A transition table encoding exactly which `(from, to)` pairs are
...
- [x] The four terminal states (`CONFIRMED`, `REFUTED`, `INCONCLUSIVE`,
      `NULL`) are reachable only from `ADVERSARIAL_REVIEW`, and once
      reached, no further transition is legal (terminal).
- [x] `Registry` class: `create(title, statement, metadata=None) -> Claim`
...
- [x] ID generation: `DISC-{year}-{5-digit sequential}`, unique, sequence
      resets per calendar year, no reuse even across process restarts
      (derive next sequence from persisted state, not an in-memory
      counter).
- [x] Persistence: JSON files under `data/claims/`, one file per claim
...
- [x] Illegal transition raises a specific exception
      (`IllegalTransitionError` or similar), not a silent no-op and not
      a bare `Exception`.

## Tests (must all pass)

- [x] Full legal sequence `DRAFT → PRE_REGISTERED → LOCKED → RUNNING →
      RESULT → ADVERSARIAL_REVIEW → CONFIRMED` succeeds and each step is
      recorded in `history` with a timestamp.
- [x] Skipping a state (e.g. `DRAFT → LOCKED`) raises
      `IllegalTransitionError`.
- [x] Transitioning out of a terminal state raises
      `IllegalTransitionError`.
- [x] Persistence round-trip: create + advance a claim, instantiate a
      fresh `Registry` pointed at the same `data/` dir, `get()` returns
      an equal `Claim` (same id, state, full history).
- [x] Two claims created in the same year get distinct, sequential ids;
      restarting the process and creating a third claim continues the
      sequence correctly (does not restart at 1 or collide).
- [x] `list(state=...)` and `list(tag=...)` filter correctly against a
      small fixture set of claims in different states.

## Acceptance

- [x] `pytest tests/test_registry.py tests/test_claim_state_machine.py -v`
      passes with zero failures and zero skips.
- [x] No use of `datetime.now()`/`time.time()` results compared without
      tolerance in tests (freeze or inject time where tests assert on
      ordering, to avoid flaky tests).
- [x] Module has no import from `05_DISCOVERY_LAB` — this is a generic
