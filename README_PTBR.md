...
**Conjectura 1 — a lei distribucional completa, agora provada em K=1, 2, 3, 4 (2026-08-25).** Além da média `φ_∞(c)` acima, o arquivo conjectura a *densidade completa* da massa cíclica condicional a exatamente `K` reroteamentos: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` fechou na onda 14; `K=3` fechou inesperadamente na onda 15, contrariando o diagnóstico de duas frentes anteriores de que a combinatória de configurações de destino explodiria além de `K=2` — em vez disso, um fato estrutural ("redirecionamentos fora-do-ciclo contribuem massa cíclica nova exatamente zero") colapsou a contagem bruta de configurações a um número pequeno e tratável de formas de ciclo também em `K=3`. `K=4` fechou na onda 16 — uma segunda surpresa consecutiva: os Bell(4)=15 padrões de coincidência colapsam a apenas 5 formas cujos pesos `∏(b_j−1)!` somam exatamente `K!=24` via uma bijeção partição↔permutação, e a identidade de floresta ponderada `E(E+Q)^{n−1}=E` por trás do cancelamento fora-do-ciclo é agora a rota candidata nomeada para `K≥5` geral (aberto, não tentado). Cada instância foi verificada por um referee hostil independente, com zero erros matemáticos encontrados:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Gráfico das quatro instâncias provadas da densidade f_M_K(x) da Conjectura 1 para K=1, 2, 3 e 4" width="620"></p>

**A constante nítida — supremo agora PROVADO: `sup_K M_K/√K = a*` (2026-08-25).** `THEOREM.md` prova `lim_{K→∞} M_K/√K = a* ≈ 0,367` exatamente (Estágio 13) e, na terceira tentativa após duas rotas fracassadas, a frente (b) da onda 16 fechou também o supremo: `M_K < a*√K` *estritamente* para todo `K≥1`, de modo que o supremo é igual ao limite (Estágio 19 — via Robbins 1955, a cota bilateral de Flajolet–Grabner–Kirschenhofer–Prodinger para a função `Q` de Ramanujan e uma identidade exata clássica; o caso de fronteira foi fechado pelo próprio referee hostil e re-verificado de forma independente). A Hipótese (U') agora carrega a constante nítida `a*` no lugar da não-nítida `a=1+√(π/2)≈2,253`. A única peça restante nesta sublinha agora está totalmente **FECHADA**: a montagem do Estágio 12 foi re-executada com `a*` no mesmo dia (onda 17), fornecendo `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0,2805]/n` para `n≥4`, `0≤c≤n`, estrita em `(0,n]`, com a constante aditiva inalterada (independência estrutural da metade da mistura de Poisson em relação a (U')) e agora certificada em aritmética racional pura — e a cota é assintoticamente justa ao longo de `c=n`, de modo que `a*` é a melhor constante multiplicativa possível para essa forma de cota:
...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (subconjunto adjudicado, `DISC-CORE-NUMERICS-001`) | **~97%** | ✅ Lema Aberto e conjectura de taxa para `K` geral provados **incondicionalmente para todo `K≥0`** (2026-08-22); a forma fechada exata, finita, em todas as ordens, para `K` geral, da recursão subjacente também está **provada** (2026-08-23), a convergência `φ(n,c)→φ_∞(c)` é provada **uniforme em `c` em toda a faixa `[0,∞)`**, não apenas pontual (2026-08-23), e uma **taxa de convergência explícita e incondicional** `|Δ_n(c)|≤[(1+√(π/2))√c+0,2805]/n` também está agora provada (2026-08-23, constante não-nítida) — todo resultado anterior em `n` finito nesta linha agora é um corolário de uma única fórmula | Restante, nenhum central: o resíduo M-CLUST(b) (um mecanismo separado, `PARTIALLY CLOSED`); a lei distribucional completa (Conjectura 1 agora provada em `K=1,2,3,4` desde 2026-08-25 — os fechamentos em `K=3` e `K=4` foram surpresas consecutivas contra um diagnóstico de explosão combinatória; `K≥5` aberto com uma rota candidata nomeada, a identidade de floresta ponderada do Estágio 20; a rota direta da Conjectura 2 honestamente NÃO fechada na onda 16 — arquitetura do método dos momentos construída, o atalho de Poissonização-em-`c` *refutado* por contraexemplo exato, e a obstrução genuína localizada na exploração conjunta de dois pontos); uma forma fechada para as constantes de erro nítidas (agora provada e executada para `p=1,...,20` em todo `b≥0` desde 2026-08-25, verificação uniforme de 75,899 checagens, com a cota de grau `deg_r H_{2k−1}=k−1` provada como bônus do referee — `p>20` aberto só por não ter sido executado, não por incerteza matemática); a taxa explícita nítida em `c` — agora totalmente **FECHADA**: o supremo `sup_K M_K/√K=a*≈0,367` foi PROVADO em 2026-08-25 (terceira tentativa), e a re-montagem do Estágio 12 com `a*` foi então executada e adversarialmente verificada no mesmo dia (onda 17): `|φ(n,c)−φ_∞(c)| ≤ [a*√c+0,2805]/n` para `n≥4`, `0≤c≤n`, estrita, com a constante aditiva inalterada e certificada em aritmética racional pura, e a cota assintoticamente justa ao longo de `c=n` — ver [mapa de dependência](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
