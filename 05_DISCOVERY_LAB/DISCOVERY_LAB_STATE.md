...
**Última atualização:** 2026-08-21 (fechamento de `transfer_entropy`, `DISC-TRI-RG-001` — 15º candidato da linha sem invariante cross-domain sobrevivente; achado forte em dado real de terremoto refutado por reprodução adversarial que descobriu um artefato instrumental de baixa frequência, não sinal sísmico genuíno)
...
| Próxima ação obrigatória | Nenhuma obrigatória. `DISC-TRI-RG-001`: Fase 0.8 tem 1/2 candidatos fechados (`transfer_entropy`, NEGATIVO — achado forte em dado real refutado por artefato instrumental descoberto na reprodução adversarial). Resta 1 candidato formalizado `viable=true` ainda não travado: complexidade de ε-machines (`C_mu`) [rank 2 — provado matematicamente distinto do `C_JS` já fechado, mas compartilha a mesma estratégia de detecção; domínio do genoma de *E. coli* é espacial, não temporal, precisa de aval explícito]. Aguardando escolha do usuário sobre fechar `C_mu` em seguida (encerrando toda a Fase 0.8), ou considerar a linha suficientemente explorada por ora (15/15 candidatos identificados com resultado final, nenhum invariante sobrevivente); status permanece `CANDIDATE_FORMULATING`. Ver `02_TESTS/TRI_RG/phase0/PHASE0_8_SURVEY_NEW_CANDIDATES.md` e `02_TESTS/TRI_RG/transfer_entropy/RESULTS_SUMMARY.md` |
