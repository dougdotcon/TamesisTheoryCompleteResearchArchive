...
**Última atualização:** 2026-08-21 (`DISC-DEC-011`: `DISC-TRI-RG-001` REABERTA a pedido explícito do usuário, com escopo estritamente delimitado — revisitar `epsilon-machine-complexity` com uma implementação CSSR incremental completa, resolvendo a ambiguidade já nomeada no fechamento original entre limitação de implementação e fragilidade genuína do estimador `C_mu`; `status` mudado de `CLOSED_NULL` de volta para `CANDIDATE_FORMULATING`; os outros 15 candidatos permanecem com veredito final documentado)
...
| Próxima ação obrigatória | Reimplementar CSSR incremental completo (Shalizi & Klinkner 2004) em `epsilon_machine_complexity/analysis/em_common.py`, substituindo o clustering guloso de `L` fixo. `I(X)` (`C_mu` primário, `h_mu` companheiro/diagnóstico) e o protocolo de significância da `METHODOLOGY_NOTE.md` original permanecem inalterados. Revalidar contra dado sintético (incluindo reexecutar o Processo Even, que deve agora convergir para os 2 estados verdadeiros, não 3). Prosseguir aos 2 domínios reais já verificados (Old Faithful, La Palma 2021) apenas se a validação mostrar poder discriminativo real. Escopo estritamente delimitado a este candidato — os outros 15 permanecem fechados. Ver `02_TESTS/TRI_RG/epsilon_machine_complexity/VALIDATION_NOTE.md` para a limitação original nomeada |
