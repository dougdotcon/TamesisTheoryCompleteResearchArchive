...
|---:|---|---:|---|---|
| 🥇 | **D —— `U₁/₂`**(已裁定子集,`DISC-CORE-NUMERICS-001`) | **约 96%** | ✅ 开放引理与一般 `K` 的速率猜想已**对每一个 `K≥0` 无条件证明**(2026-08-22);关于底层递归式、适用于一般 `K` 的精确、有限的全阶闭式表达式现已同样得到**证明**(2026-08-23),收敛 `φ(n,c)→φ_∞(c)` 已被证明在 **`c` 取遍整个范围 `[0,∞)` 时一致成立**,而不仅仅是逐点成立(2026-08-23),且一个**显式、无条件的收敛速率** `|Δ_n(c)|≤[(1+√(π/2))√c+0.2805]/n` 现已同样得到证明(2026-08-23,非最优常数)——本研究线此前所有有限 `n` 的结果现均成为这一单一公式的推论 | 剩余事项,均非核心:M-CLUST(b) 残差(一个独立的机制,`PARTIALLY CLOSED`);完整的分布律(猜想 1–2);`b≥2` 情形下精确误差常数的闭式表达式;上述显式速率中的**最优(sharp)常数**(已证明的非最优值 `a≈2.253`,对比 `a*≈0.367`)——见[依赖关系图](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
