...
**Última atualização:** 2026-08-21 (`DISC-DEC-012`: revisão delimitada de `epsilon-machine-complexity` concluída — CSSR incremental completo confirma, de forma decisiva, que a falta de poder de `C_mu` é fragilidade genuína do estimador, não limitação de implementação; `DISC-TRI-RG-001` ENCERRADA FORMALMENTE novamente, `status: CANDIDATE_FORMULATING -> CLOSED_NULL`)
