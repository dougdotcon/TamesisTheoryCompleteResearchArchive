...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (adjudicated subset, `DISC-CORE-NUMERICS-001`) | **~94%** | ✅ Open Lemma and general-`K` rate conjecture proved **unconditionally for every `K≥0`** (2026-08-22); the exact, finite, all-orders, general-`K` closed form for the underlying recursion is now **proved** as well (2026-08-23), and the convergence `φ(n,c)→φ_∞(c)` is proved **uniform in `c` on the entire range `[0,∞)`**, not just pointwise (2026-08-23) — every earlier finite-`n` result on this line is a corollary of one formula | Remaining, none central: the M-CLUST(b) residual (a separate mechanism, `PARTIALLY CLOSED`); the full distributional law (Conjectures 1–2); a closed form for the sharp error constants at `b≥2`; an explicit convergence rate (conditional on one named, unproved uniformity hypothesis) — see [dependency map](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
