...
| コア数値判定 | **数学的統合が完了、`K=10`まで無条件でギャップを解消(2026-08-22)** | 下記参照 ― 3件の主張が最終判定とともに否定的にクローズされた。4件目(`U₁/₂`)は、証明済みの中核部分を持ち敵対的に査読済みであり、未解決補題(Open Lemma)は現在 `K=0,…,10` について無条件に証明されている。一般の `K` については、レート予想の条件付き証明が存在する(敵対的査読者によって正しく範囲設定されたと判断された正則性に関する留保条件があり)、無条件のクロージャではない。 |
...
| **`U₁/₂` 極限法則(第2〜7波、統合)** | 定理1件 + 一般化1件 + 未解決補題(Open Lemma)の `K=2,…,10` ケース + 一般 `K` に対するレート予想 | **証明済み、敵対的査読済み(独立した3ラウンド、異なる手法)、論文+再現可能パッケージとして公開。`K=2` は第5波、`K=3,4,5` は第6波、`K=6,…,10` は第7波で証明され、いずれも転送行列法による。一般 `K` のレートは第7波でPROVED(証明済み)となったが、正則性に関する留保条件に明示的に条件付けられる**(下記参照) |
...
有限モデルと極限対象の間の橋渡しは、現在 `K=0,…,10` について無条件に証明されており、一般 `K` に対するレート予想は、敵対的査読者によって正確に範囲設定された正則性仮説を条件として証明されている:
...
flowchart LR
    ...
    K610 --> Kgen["K一般<br/>第7波:レートPROVED(証明済み)、<br/>1件の名指しされた留保条件付き"]
    Kgen -.->|"第8波、進行中"| Close["留保条件を除去 →<br/>すべてのKについて無条件化"]
...
上記の各段は、それぞれ独立した敵対的査読者エージェントによって、元の導出とは*異なる*証明手法、独自のブルートフォース列挙、および完全な再帰代入チェックを用いて独立に再導出された ― 独立した3ラウンドの査読を通じて、**いかなる層でも誤りはゼロ件**であった。`K=6,…,10` については、2つのホールドアウト点における新規の網羅的列挙とビット単位で一致することもさらに確認された。残る唯一の**未解決補題(Open Lemma)**― `K=10` まで無条件に証明され、一般 `K` については条件付きで証明されている ― は、Hansen & Jaworski(EJC、2014年)の厳密な固定パラメータケースである。閉形式の `erf` を伴うポアソン混合は、体系的な文献調査(35件以上のクエリを記録)では見つからなかったが、これが「新規性」と同義ではないという明示的な留保が付されている。第二の戦線では、**なぜ指数がちょうど1/2なのか**が導出された:摂動機構のパラメトリックな族全体にわたって、`α ∈ [1/2, 1]` が常に成り立ち ― `α < 1/2` は*証明により不可能*であることが示された(いかなる周期性の「消滅」がなくても持続する二次的クラスタリング効果による)。第5波では、中間のあらゆる `α ∈ (1/2, 1)` に到達する自然な機構(`M-WEIB(β)`、非斉次ワイブル・ハザード)も特定・確認された。物理的な含意は一切主張されていない ― これは特定のアンサンブルに関する純粋な組合せ数学である。
...
|---:|---|---:|---|---|
| 🥇 | **D ― `U₁/₂`**(判定済みサブセット、`DISC-CORE-NUMERICS-001`) | **~85%** | 🔥 進行中 ― 未解決補題(Open Lemma)は `K=0,…,10` について無条件に証明済み、一般 `K` のレートは条件付きで証明済み | 一般 `K` の正則性に関する留保条件、**および** M-CLUST残差の解消 ― 現在進行中の2つの戦線([依存関係マップ](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md)参照) |
...
**この表の誤った使い方。** 「85%」は、`U₁/₂` クラスが正しい確率が85%であることも、ラインDがほぼ完了に近いことも意味しない ― それが意味するのは、その特定の問いにおいて明示的に名指しされたギャップのうち、大半がすでに証明されたか、精密に特徴づけられているということである。「今どこに研究の労力を投じるべきか」を基準とするならば、答えはすでに研究室を導いているものと同じである:利用可能な研究能力の大半は `D — U₁/₂` に投じられ、すでに進行中の2つの戦線 ― M-CLUST残差の解消と、一般 `K` の正則性に関する留保条件の除去 ― にちょうど分かれている。
