...
front de rascunho — mandato `DISC-DEC-027(b)`, item `EEG-DEPRESSION-PREREG-DRAFT`)
**Test ID:** `DISC-COGNITIVE-EEG-SPECTRAL-001` (braço depressão, sub-registro)
**Commit em que foi travado:** N/A — este é um RASCUNHO. Nenhum commit de lock
foi feito por este agente. Autorizado apenas a redigir (`DISC-DEC-027`); o
lock (commit) é uma decisão explícita e auditável reservada à sessão
orquestradora, numa decisão subsequente, depois de resolver as duas lacunas
abertas abaixo (§8, §2.1).

> **Este documento NÃO está travado.** Nenhum dado real de sinal EEG foi
> baixado, aberto, ou computado para produzir este rascunho — ver §11. Nenhuma
> mudança foi feita em `OPERATIONALIZATION.md`, `DECISION_LEDGER.yaml`, ou
> `TEST_QUEUE.yaml`. Este arquivo consolida, com resolução explícita das duas
> lacunas nomeadas por `DISC-DEC-026`/`DISC-DEC-027`, o conteúdo já
> fixado em `OPERATIONALIZATION.md`, restrito ao braço depressão (Mumtaz).
