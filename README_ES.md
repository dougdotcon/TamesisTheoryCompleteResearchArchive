...
**Conjetura 1 — la ley distribucional completa, ahora probada en K=1, 2, 3 (2026-08-24).** Además de la media `φ_∞(c)` anterior, el archivo conjetura la *densidad completa* de la masa cíclica condicional a exactamente `K` reencaminamientos: `f_{M_K}(x) = 2Kx(1-x²)^(K-1)`. `K=2` se cerró en la onda 14; `K=3` se cerró inesperadamente en la onda 15, contradiciendo el diagnóstico de dos frentes anteriores de que la combinatoria de configuraciones de destino explotaría más allá de `K=2` — en cambio, un hecho estructural ("los redireccionamientos fuera del ciclo contribuyen exactamente cero masa cíclica nueva") colapsó el recuento bruto de configuraciones a un número pequeño y manejable de formas de ciclo también en `K=3`. Cada instancia fue verificada por un árbitro hostil independiente, sin ningún error matemático encontrado:

<p align="center"><img src="05_DISCOVERY_LAB/assets/conjecture1_densities.svg" alt="Gráfico de las tres instancias probadas de la densidad f_M_K(x) de la Conjetura 1 para K=1, 2 y 3" width="620"></p>

**La cuestión de la constante nítida — límite probado, supremo abierto.** `THEOREM.md` también prueba `lim_{K→∞} M_K/√K = a* ≈ 0,367` exactamente (Estágio 13) — la primera confirmación rigurosa de que la constante nítida conjeturada numéricamente es genuinamente el valor asintótico correcto. Si `sup_K M_K/√K` es igual a ese mismo límite (es decir, si `M_K/√K` nunca supera `a*` en ningún `K` finito) sigue abierto — dos rutas ya han fallado, y una tercera está en curso ahora (onda 16, frente b):

<p align="center"><img src="05_DISCOVERY_LAB/assets/sharp_constant_astar.svg" alt="Gráfico de M_K sobre la raíz cuadrada de K aproximándose, pero sin prueba de estar acotado por la constante nítida a-estrella" width="620"></p>
...
|---:|---|---:|---|---|
| 🥇 | **D — `U₁/₂`** (subconjunto adjudicado, `DISC-CORE-NUMERICS-001`) | **~97%** | ✅ Lema Abierto y conjetura de tasa para `K` general probados **incondicionalmente para todo `K≥0`** (2026-08-22); la forma cerrada exacta, finita, a todos los órdenes, para `K` general, de la recursión subyacente también está **probada** (2026-08-23), la convergencia `φ(n,c)→φ_∞(c)` está probada **uniforme en `c` en todo el rango `[0,∞)`**, no solo puntual (2026-08-23), y una **tasa de convergencia explícita e incondicional** `|Δ_n(c)|≤[(1+√(π/2))√c+0,2805]/n` también está ahora probada (2026-08-23, constante no nítida) — todo resultado anterior en `n` finito de esta línea es ahora un corolario de una única fórmula | Restante, ninguno central: el residuo M-CLUST(b) (un mecanismo separado, `PARTIALLY CLOSED`); la ley distribucional completa (Conjeturas 1–2, ahora probada en `K=1,2,3` desde 2026-08-24 — el cierre en `K=3` fue inesperado, contradiciendo un diagnóstico de explosión combinatoria compartido por dos frentes anteriores; `K≥4` abierto, no intentado); una forma cerrada para las constantes de error precisas (ahora probada para `p=1,...,10` en todo `b≥0` desde 2026-08-24, con la máquina general-`k` subyacente probada correcta para todo `k` por inducción — `p>10` abierto solo por no haberse ejecutado, no por incertidumbre matemática); la constante **nítida** en la tasa explícita anterior (`a*≈0,367` frente al valor no nítido `a≈2,253` ya probado, aunque el *límite* `lim_K M_K/√K=a*` ya está ahora probado exactamente, 2026-08-23 — solo queda abierto el supremo uniforme-en-`K`) — ver [mapa de dependencias](05_DISCOVERY_LAB/02_TESTS/CORE_NUMERICS/u12_universality/PROOF_DEPENDENCY_MAP.md) |
