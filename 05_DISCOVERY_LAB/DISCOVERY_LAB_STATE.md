...
**Última atualização:** 2026-08-20 (Fase 0.8 de `DISC-TRI-RG-001`: nova rodada de busca a pedido do usuário, reabrindo a linha logo após `DISC-DEC-009`; 2 novos candidatos `viable=true` — transferência de entropia, complexidade de ε-machines — os primeiros de toda a linha a não caírem nos 4 eixos matemáticos já testados repetidamente)
...
| Próxima ação obrigatória | Nenhuma obrigatória. `DISC-TRI-RG-001`: Fase 0.8 tem 2 candidatos formalizados `viable=true` ainda não travados: transferência de entropia [rank 1 — primeira candidatura bivariada/direcional de toda a linha, R_lambda deliberadamente escolhido para evitar o modo de falha de embedding que já fechou RQA/LLE] e complexidade de ε-machines [rank 2 — provado matematicamente distinto do `C_JS` já fechado, mas compartilha a mesma estratégia de detecção]. Aguardando escolha do usuário sobre qual fechar primeiro, ou considerar a linha suficientemente explorada por ora; status permanece `CANDIDATE_FORMULATING`. Ver `02_TESTS/TRI_RG/phase0/PHASE0_8_SURVEY_NEW_CANDIDATES.md` |
