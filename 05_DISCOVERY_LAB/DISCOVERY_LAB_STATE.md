...
**Última atualização:** 2026-08-22 (`DISC-DEC-017`: onda 3 de `DISC-DEC-015` integrada por completo — teorema rigoroso de U₁/₂ verificado por referee adversarial [nenhum erro encontrado], generalização U_α derivada e confirmada [α∈[1/2,1] para toda a classe, α<1/2 provado impossível], busca de prioridade concluída [mistura Poisson/erf segue sem antecedente encontrado], pacote `tamesis-cycle-survival/` publicado com paper compilado + 49/49 testes passando, `FAILED_HYPOTHESES.md` publicado na raiz. Único gap real [ponte n→∞ para K≥2] honestamente aberto, documentado, não escondido)
