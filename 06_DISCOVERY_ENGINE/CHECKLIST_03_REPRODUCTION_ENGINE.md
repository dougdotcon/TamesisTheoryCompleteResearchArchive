...
- [x] `Reproducer.reproduce(claim_id, second_test_plan, tolerance=1e-9) ->
...
- [x] Numeric comparison must support: scalars (abs/rel tolerance),
      ...
      by equality.
- [x] Three possible verdicts, not two: `EXACT_MATCH` (bit-identical),
...
- [x] `ReproductionRecord` persisted (JSON under `data/reproductions/`),
      linked to claim id, storing: verdict, per-field deltas, the
      tolerance used, timestamp.
- [x] A guard used by Module 4/5: `has_successful_reproduction(claim_id)
...
- [x] Tolerance boundary behavior: a delta of exactly `tolerance` (or
      ...
      case.
- [x] `has_successful_reproduction()` returns `False` before any
...
- [x] `pytest tests/test_reproduction.py -v` passes with zero failures.
- [x] The mismatch-detection test is not tautological — it must use a
